...
class Model(nn.Module):
    ...
    def forward(self, input1,input2,return_embed=False):
        ...
        if self.mode==0:
            ...
            x1=torch.cat([self.avgpool(emb1).squeeze(),self.avgpool(emb1).squeeze()],dim=-1)
            x2=torch.cat([self.avgpool(emb2).squeeze(),self.avgpool(emb2).squeeze()],dim=-1)
...
# # baseline=[]
model1=Model(backbone=backbone1,embed_size=EMBED_SIZE*2,mode=4).to(DEVICE)
# model3=Model(backbone=backbone1,embed_size=EMBED_SIZE,mode=3).to(DEVICE)
baseline=[model1]
baseline_name=['resnet']
...
for idx, model in enumerate(baseline):
    ...
    model_list=[Model(backbone=backbone1,embed_size=EMBED_SIZE*2,mode=4).to(DEVICE),Model(backbone=backbone1,embed_size=EMBED_SIZE*2,mode=4).to(DEVICE),Model(backbone=backbone1,embed_size=EMBED_SIZE*2,mode=4).to(DEVICE)]
